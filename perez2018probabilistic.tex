...
We need to find an optimal number k for our model. We will use the CRPS of the prodeicted distribution
...
We will use the quantile random forest as described in . Quantiles are built from the observations in 
the training set. We take the observation points that belong to the same leaf than the input and we build a 
distribution from those points. See figure for explanation:
